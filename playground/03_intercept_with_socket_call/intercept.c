 ...
 * then performs the actual read via syscall(SYS_write,...).
 *
...
#include <stdio.h>
#include <unistd.h>
#include <sys/syscall.h>   // syscall numbers (SYS_write)
#include <sys/types.h>
#include <stdint.h>
#include <string.h>
#include <errno.h>
...
#define SOCKET_PATH "/tmp/example_socket_intercept_call"
...
static __thread int in_write_intercept = 0; /* thread-local reentry guard */
...
ssize_t write(int fd, void *buf, size_t count) {
    ...
    if (in_write_intercept) {
        return syscall(SYS_write, fd, buf, count);
    }

    in_write_intercept = 1; /* enter guarded region */
