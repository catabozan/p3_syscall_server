...
\\
  ...
  This has applications in confidential computing environments\cite{confidential_computing} where a target program is running in a `sealed' virtual environment, but still needs some access to the host's Linux kernel. 
\\
...
Fortunately if you build sqlite3 from source, you can disable the mmap() syscall by using specific compilation flags. After compiling sqlite3 from source with mmap() disabled I was able to get a list of syscalls I could intercept.\cite{sqlite_compile}
\\
...
They all have their own advantages and disadvantages, I ultimately chose SunRPC + XDR\cite{sun_rpc} since it's lightweight, fast and widely used.
...
\begin{description}
  ...
  \item[\texttt{\detokenize{sqlite3}}]
  - sqlite3 binary compiled without mmap().
...
In order to use the client you must load the compiled shared library before all other shared libraries, by setting the \textit{\detokenize{LD_PRELOAD=build/intercept.so}}\cite{syscall_intercept} environment variable.
\\
...
To get to this list I have run the \textit{strace} command.
...
\subsection{Syscall list and function}\cite{linux_man}
\begin{itemize}
...
\subsection{Individual syscalls}

To test individual syscalls, I've created a test program: \textit{\detokenize{src/program.c}}.
\\

The program has individual test functions for each syscall I've implemented that are ran sequentially in the main function.
Here is an example:

\begin{lstlisting}[style=mycode, language=C, caption=\detokenize{Test function example in program.c}] 
...
\subsubsection{Test program syscall interception output}

Here you have what is displayed in the terminal when the \textit{src/program.c} runs with syscall interception.
\\
...
\subsection{Running sqlite3}

To run sqlite3, the manually compiled \textit{sqlite3} binary must be used, because it has been compiled to not use mmap().
\\
...
[Client] access() RPC result: -1, errno=2
sqlite> CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL);
[Client] Intercepted access("test.db", 0)
[Client] access() RPC result: -1, errno=2
...
\subsection{Sqlite3 interception}
...
Queries seem to run fine, but when trying to fetch data, for example, sqlite throws an error: \textit{\detokenize{Runtime error: database disk image is malformed (11)}}.
\\
...
\subsubsection{Minimizing external influences}
...
\subsubsection{Minimizing print statements}
...
This is accomplished by compiling and preloading another shared library. Its source code resides in \textit{\detokenize{src/disable_print.c}} and its shared library is \textit{\detokenize{build/no_print.so}}.
\\
...
I have used the \textit{time} command to time process execution. 
\\
...
\pgfplotstableread[col sep=comma]{benchmarks/benchmarks.csv}\datatable

\begin{table}[h!]
\centering
\pgfplotstabletypeset[
    columns/ITERATIONS/.style={string type},
    columns/NO INTERCEPT/.style={string type},
    columns/WITH INTERCEPT/.style={string type},
    columns/DELTA/.style={string type},
    columns/PERFORMANCE LOSS/.style={string type},
    every head row/.style={before row=\toprule,after row=\midrule},
    every last row/.style={after row=\bottomrule},
]{\datatable}
\caption{Performance comparison with and without intercept.}
\label{tab:performance_loss}
\end{table}
...
While the implementation intercepted and delegated 18 common syscalls, enabling the execution of basic test programs and a modified version of sqlite3, it also revealed a lot of challenges. 
Benchmarking indicated a substantial performance loss, with syscall delegation resulting in a performance loss of roughly 5.5x compared to direct execution. 
Furthermore, unresolved technical issues remain, such as the server's inability to handle multiple sequential client connections without a restart and a persistent data corruption error in sqlite3 likely linked to the lseek implementation.
